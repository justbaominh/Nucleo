...
void I2C1_InitByteWrite(uint8_t address, uint8_t command);
...
int main(void)
{
    ...
    */
    ...
    uint8_t air_quality_command = 0x08; // read last result command 1010 1010

    uint8_t received_data[6] = {0}; // data array for temperature
    /* Infinite loop */
    uint8_t buf[50] = "start";

    I2C1_InitByteWrite(address_for_write, start_conversion); // start conversion
    ...
    while (1)
    {
        I2C1_Read(address_for_read, air_quality_command, 6, received_data);

        send_to_USART2((int)received_data[0], (int)received_data[1]);
        send_to_USART2((int)received_data[2], (int)received_data[3]);
...
void I2C1_InitByteWrite(uint8_t address, uint8_t command)
{
    ...
    I2C1->DR = 0x2003; // send feature set MSB
    // I2C1->DR = 0x03;		//send command LSB
...
void I2C1_Read(uint8_t address, uint8_t command, int n, uint8_t *data)
{
    ...
    I2C1->DR = 0x2008; // send feature set MSB
    // I2C1->DR = 0x08;
    while (!(I2C1->SR1 & 0x80))
    {
    } // wait until data register empty p.689
